...
- Use [Azure Data Factory](https://azure.microsoft.com/services/data-factory) for orchestration instead of Azure Synapse pipelines. The choice depends on several factors:
  ...
  For more information and a feature comparison between Azure Synapse pipelines and Data Factory, see [Data integration in Azure Synapse Analytics versus Azure Data Factory](https://docs.microsoft.com/azure/synapse-analytics/data-integration/concepts-data-factory-differences).
...
10. In **New linked service (OData)**, enter a name and description for the query data you’re linking to.
     ...
     a. In **Connect via integration runtime**, select **AutoResolveIntegrationRuntime**.
     
     b. Open Workplace Analytics in a separate browser, and then on the Query designer **Results** page, copy the OData link for the person query data you want to connect to Azure.
     
     c. In **Service URL**, paste the query OData link that you copied in the previous step.
     
     d. In **Authentication type**, select either **AAD service principal with Key** or **AAD service principal with Cert**. For details about these options, see [Use Azure Key Vault secrets in pipeline activities](https://docs.microsoft.com/azure/data-factory/how-to-use-azure-key-vault-secrets-pipeline-activities).
     
     e. In **AAD resource**, enter `https://workplaceanalytics.office.com`.
     
     f. If not set by default, paste your saved **Directory ID** (tenant) in **AAD tenant**.
     
     g. Paste your saved **Application ID** (client) recorded in **AAD service principal ID**. For details, see [Linked service properties](https://docs.microsoft.com/azure/data-factory/connector-odata#linked-service-properties).
     
     h. In **Azure Active Directory** > **your newly registered analytics app**, select **Certificates & secrets**, and then do one of the     following.
    ...
    i. Back in the **New linked service** window, do the following for the applicable authentication type:
    ...
    j. Select **Test connection** to test the OData linked service.
    
    k. After confirming that the connection is successful, select **Create**.
